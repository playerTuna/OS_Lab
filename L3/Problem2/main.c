...
void* sum_worker(void* arg) {
   ...
   // Ghi vào biến toàn cục được bảo vệ bằng mutex
...
int main(int argc, char* argv[]) {
   ...
   thread_idx_range = (struct _range*)malloc(appconf.tnum * sizeof(struct _range));
   ...
   /* Generate array data */
   shrdarrbuf = (int*)malloc(appconf.arrsz * sizeof(int));
   if (shrdarrbuf == NULL) {
    ...
    */
   pthread_mutex_init(&mtx, NULL);  // Đừng quên khởi tạo mutex

   tid = (pthread_t*)malloc(appconf.tnum * sizeof(pthread_t));
   ...
   pthread_mutex_destroy(&mtx);  // Hủy mutex sau khi xong
