...
void* sum_worker(void* arg) {
   struct _range* idx_range_ptr = (struct _range*)arg;
   int i;
   printf("In worker from %d to %d\n", idx_range_ptr->start, idx_range_ptr->end);
   for (i = idx_range_ptr->start; i <= idx_range_ptr->end; i++) {
      pthread_mutex_lock(&mtx);
      sumbuf += shrdarrbuf[i];
      pthread_mutex_unlock(&mtx);
   }
   printf("Worker from %d to %d done with result %ld\n", idx_range_ptr->start, idx_range_ptr->end, sumbuf);
   return NULL;
}
