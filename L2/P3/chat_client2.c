...
#define PERMS 0644
#define MSG_KEY_1_TO_2 0x123  // Hàng đợi để nhận tin từ client1
#define MSG_KEY_2_TO_1 0x124  // Hàng đợi để gửi tin đến client1
    ...
    while (!should_exit) {
        ...
        if (msgrcv(msqid, &msg, sizeof(msg.mtext), 0, IPC_NOWAIT) == -1) {
            ...
            if (strcmp(msg.mtext, "exit") == 0) {
                printf("Client1 has exited. Press Enter to exit too.\n");
                should_exit = 1;
                break;
            }
            printf("You: ");
...
int main() {
    ...
    if ((recv_msqid = msgget(MSG_KEY_1_TO_2, PERMS | IPC_CREAT)) == -1) {
        perror("msgget (receive)");
        exit(1);
    }

    // Tạo hoặc kết nối tới message queue để gửi tin
    if ((send_msqid = msgget(MSG_KEY_2_TO_1, PERMS | IPC_CREAT)) == -1) {
        perror("msgget (send)");
        exit(1);
    }

    printf("=== Client 2 Chat Program ===\n");
    ...
    // Dọn dẹp message queues khi kết thúc
    if (should_exit && msgctl(send_msqid, IPC_RMID, NULL) == -1) {
        perror("msgctl (cleanup send queue)");
    }
    if (should_exit && msgctl(recv_msqid, IPC_RMID, NULL) == -1) {
        perror("msgctl (cleanup receive queue)");
    }
